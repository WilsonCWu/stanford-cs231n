...
def layernorm_backward(dout, cache):
    ...
    dgamma = np.sum(dout * x_normalized, axis=0)
    dx_normalized = dout * gamma
    dx = (dx_normalized - np.mean(dx_normalized, axis=1, keepdims=True) - x_normalized * np.mean(dx_normalized * x_normalized, axis=1, keepdims=True)) / np.sqrt(sample_var + eps)
